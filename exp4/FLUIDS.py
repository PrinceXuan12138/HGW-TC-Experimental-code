...
latent_dim = 39
verbose, epochs, batch_size =2,5, 64  # Parameters for local training
numOfIterations=50   #global epochs
numOfClients=10  #num of sub nodes
monitoring_filename= "./result/FLUIDS_label_性能监控.csv"   #Path of the performance monitoring record file
performance_filename= "./result/FLUIDS_label_评价指标.csv"  #Path of the performance  index record file
AEmodelLocation="./Models/FLUIDS_AE_"+str(numOfClients)+"_nodes.h5"   #Path of the server model
ClassficationmodelLocation="./Models/FLUIDS_CF_"+str(numOfClients)+"_nodes.h5" #Path of the server model
...
# ------- 2. The training data is split according to the number of sub nodes---------
    ...
    for clientID in range(numOfClients):
        # because FLUIDS is locally unsupervised learning, there is no filtering of tagged data here, all for training
